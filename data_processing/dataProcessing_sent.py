...
def calc_vec(sents):
	...
	all_vecs = bc.encode([sents[i].split('\t')[0] for i in range(len(sents))])
	# print(len(vec))
	# print(len(vec[0]))
	# print(len(vec[1]))
	# print(len(vec[0][0]))
...
def sep_data():
	with open("ReviewRebuttalnew.txt", "r") as full_data:
		...
		for i in dev_data:
			...
			vecs.append([vec[i][0] for i in range(len(vec))])
			# print('len of vec: ', len(vecs[-1]))
		dev_vecs = open('vec_dev_sent.pkl', 'wb')
		...
		for i in test_data:
			...
			vecs.append([vec[i][0] for i in range(len(vec))])
			# print('len of vec: ', len(vecs[-1]))
		test_vecs = open('vec_test_sent.pkl', 'wb')
		...
		for i in train_data:
			...
			vecs.append([vec[i][0] for i in range(len(vec))])
			# print('len of vec: ', len(vecs[-1]))
		train_vecs = open('vec_train_sent.pkl', 'wb')
